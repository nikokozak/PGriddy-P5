...
export default class CustomGrid
{
              ...
              instanceParams: any)
  {
    ...
    this.points = [];
    populateDefaultPoints(this, className, instanceParams);
...
function populateDefaultPoints (pointGrid: CustomGrid, className: any, instanceParams: any) : void
{
...
// in a column-first flat array fashion.
  ...
  for (let i = 0; i < pointGrid.numX; i++) {
    ...
    for (let j = 0; j < pointGrid.numY; j++) {
      ...
      instanceParams.x = xPos;
      instanceParams.y = yPos;
      instanceParams.iX = i;
      instanceParams.iY = j;

      pointGrid.points.push(new className(instanceParams));
